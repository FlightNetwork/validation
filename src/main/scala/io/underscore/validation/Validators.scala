...
trait Validators {
  ...
  def fail(msg: => String): Seq[ValidationResult] =
    Seq(ValidationError(message = msg))

  def fail(code: => String, msg: => String): Seq[ValidationResult] =
    Seq(ValidationError(code, msg))

  def warn(msg: => String): Seq[ValidationResult] =
    Seq(ValidationWarning(message =  msg))

  def warn(code: => String, msg: => String): Seq[ValidationResult] =
    Seq(ValidationWarning(code, msg))
